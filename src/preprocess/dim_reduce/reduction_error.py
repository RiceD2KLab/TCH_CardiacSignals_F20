...
def mean_squared_error(reduced_dimensions, model_name, patient_num, save_errors=False):
    ...
    original_signals = np.load(os.path.join("Working_Data", "Normalized_Fixed_Dim_HBs_Idx{}.npy".format(str(patient_num))))

    reconstructed_signals = np.load(os.path.join("Working_Data", "reconstructed_{}_{}d_Idx{}.npy".format(model_name, reduced_dimensions, patient_num)))
    ...
    if save_errors:
        np.save(os.path.join("Working_Data", "{}_errors_{}d_Idx{}.npy".format(model_name, reduced_dimensions, patient_num)))
    return mse
...
def compare_patients(model_name, reduced_dimensions):
    """
    ...
    errors = {}
    for file_index in heartbeat_split.indicies:
        errors[file_index] = float(mean_squared_error(reduced_dimensions, model_name, file_index).mean())
    print(errors)
    return errors

def compare_dimensions(model_name, patient_num):
    """
    ...
    for dim in dimensions:
        dimension_errors[dim] = mean_squared_error(dim, model_name, 1).mean()

    plt.plot(dimensions, [np.mean(mse_list) for mse_list in dimension_errors.values()])
    plt.title("Mean Squared Error for Reconstructed Signal for Lead 1 of Patient 1 using the {} model".format((model_name)))
