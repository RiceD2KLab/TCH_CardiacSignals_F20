...
Last edited: 10/11/2020, by Frank Yang
...
2. Import the normalized heartbeat data
3. Run the VAE
...
def run_vae(file_index, rng, split_ratio, plot_results=False):
    """
    For desired patient index, range specifying dimensionality of latent space, and data split ratio, train the VAE on
    the first half of the data split, and evaluate the VAE on the second half
    :param file_index: integer or list of integers that specifies
    :param rng:
    :param split_ratio:
    :param plot_results:
    :return:
...
"""
MAIN FUNCTION
- iterate over the desired patient indices
- Train VAE on these patients -> specify desired dimension in latent space (as a list), and plot results
"""
if __name__ == "__main__":
    for file_index in heartbeat_split.indicies[:1]:
        run_vae(file_index, range(1, 2), 5/6, plot_results=True)
