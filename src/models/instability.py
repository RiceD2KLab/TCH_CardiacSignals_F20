...
def calculate_streamed_variances(heartbeats, window_duration, step, vae=False):
    """
    ...
    """
    heartbeat_len = heartbeats.shape[1 if vae else 0 ]
    k = heartbeats.shape[2 if vae else 1]

    variances = []
    for i in range(window_duration, heartbeat_len, step):
        window = heartbeats[2, i-window_duration:i, :] if vae else heartbeats[i-window_duration:i :]# n x k
...
def calculate_streamed_entropies(heartbeats, window_duration, step, bins, vae=False):
    heartbeat_len = heartbeats.shape[1 if vae else 0 ]
    k = heartbeats.shape[2 if vae else 1]
    entropies = []
    for i in range(window_duration, heartbeat_len, step):
        window = heartbeats[2, i-window_duration:i, :] if vae else heartbeats[i-window_duration:i :]
...
def calculate_streamed_cross_entropies(heartbeats, window_duration, step, bins, vae=False):
    heartbeat_len = heartbeats.shape[1 if vae else 0 ]
    k = heartbeats.shape[2 if vae else 1]
    cross_entropies = []
    truth = heartbeats[2, 0:window_duration, :] if vae else heartbeats[0:window_duration, :]
    for i in range(window_duration, heartbeat_len, step):
        window = heartbeats[2, i - window_duration:i, :] if vae else heartbeats[i - window_duration:i:]
        binned_dimensions = np.apply_along_axis(lambda row: np.histogram(row, bins=bins, density=True)[0], 0, window)
...
def plot_metrics(metric_name, k, patient_idx, model_name):
    ...
    is_vae = True if model_name == "vae" else False
    if metric_name == "variance":
        variances = calculate_streamed_variances(data, 500, 50, is_vae)  # assume 100 bpm with 5 min window = 500 samples
        metrics = [variance if variance < 20 else 0 for variance in variances]
    elif metric_name == "entropy":
        entropies = calculate_streamed_entropies(data, 500, 50, 20, is_vae)  # assume 100 bpm with 5 min window = 500 samples
        metrics = [entropy if entropy < 20 else 0 for entropy in entropies]
    elif metric_name == "cross entropy":
        metrics = calculate_streamed_entropies(data, 500, 50, 20, is_vae)
