...
def get_metric_differences(variances):
    ...
    second_hour = variances[int(len(variances)/6):int(2*len(variances)/6)]
    last_hour = variances[:int(5*len(variances)/6)]
    if len(last_hour)>0 and len(first_hour)>0 and len(second_hour)>0:
        ...
        _, pval_end_compare_ks = kruskal(last_hour, first_hour)
        ks_end = True if pval_end_compare_ks < .05 else False
        ks_start = True if pval_start_compare_ks < .05 else False
        isValid = True
    else:
        isValid = False
        krusk_end,krusk_start,ks_end,ks_start = None,None,None,None
    return(isValid,krusk_start,krusk_end,ks_start,ks_end)
