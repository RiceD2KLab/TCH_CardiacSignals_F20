"""
Functions to verify the results of preprocessing (i.e. peak detection/splitting)
"""
...
def verify_detected_heartrate():
    ...
    for i in range(1, 60):
        try:
            diff_list = []
            heartbeat_peaks = np.load(os.path.join("Working_Data", "HB_Peaks_Idx" + str(
                i) + ".npy"))  # indices of the detected peaks of the heartbeats in the original vector
            heartbeat_lens = np.load(os.path.join("Working_Data", "HB_Lens_Idx" + str(
                i) + ".npy"))  # length of the heartbeat IN SAMPLES (NOT SECONDS) -> vector length is one less than peaks vector
            print("for file {}".format(i))

            h5_file = h5_interface.readh5("Reference_idx_" + str(i) + "_Time_block_1.h5")
            true_heartrate = h5_file["PARM_HR"][
